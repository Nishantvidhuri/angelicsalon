...
function Pricing() {
  ...
  return (
    <div className="w-screen max-w-6xl mx-auto h-full mb-40 px-6">
      ...
      {/* Tab Navigation */}
      <div className="flex  justify-between w-[1200px] border-b  mb-24">
        {Object.keys(pricingData).map((tab) => (
            <div 
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`px-4 py-2 text-2xl font-jakarta  
         w-full border-b-2 font-medium ${
              activeTab === tab ? "text-black border-black" : "text-gray-400 "
            }`}>
      ...
      <div className="w-full">
        {pricingData[activeTab].map((item, index) => (
          <div
            key={index}
            className="flex justify-between items-center py-4 border-b border-gray-300 font-jakarta"
          >
            <span className="w-[15%] min-w-[180px] text-2xl font-medium text-left">
              {item.service}
            </span>

            {/* Wavy Line */}
            <span className="w-[35%] min-w-[250px]">
              <svg className="w-full h-8" viewBox="0 0 600 40" xmlns="http://www.w3.org/2000/svg">
                <path
            ...
            <span className="w-[10%] min-w-[100px]  text-lg font-semibold text-center">
              {item.price}
            </span>

            <span className="w-[15%] min-w-[180px] 
            text-2xl font-medium text-left">
              {item.extra}
            </span>
